...
FF_intval = 100 #value we choose for our intervention on fatty fish
...
df_long0 <- expandRows(df, count=90, count.is.col=F) #make 180 rows for 180 months of followup
df_long0$time <- rep(seq(0,89), nrow(df)) #create time variable, 0-179 months
...
df_long1 <- df_long0
df_long1$time <- rep(seq(0,89), nrow(df))
df_long1$PM[df_long1$fatfish < FF_intval] = pmax(PM-100, 0) # modelling the replacement of 100 kcal
df_long1$fatfish[df_long1$fatfish < FF_intval] = fatfish+100 # modelling the replacement of 100 kcal
...
write.csv2(gf, 'intervention_3_data.csv') # correct if needed :) 
